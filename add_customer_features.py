...
def insert_components(filepath):
    ...
    <div className="space-y-8 pb-20 md:pb-0">
      ...
      <div className="grid md:grid-cols-2 gap-6">
        <GlassCard className="p-6 border border-[#04407E]/20 bg-gradient-to-br from-[#04407E]/5 to-transparent">
          ...
          <button className="px-6 py-2 bg-[#04407E] text-white rounded-full font-semibold hover:bg-[#033060] transition-colors w-full">
            Claim Offer
          </button>
        </GlassCard>
        ...
        <GlassCard className="p-6 border border-gray-200">
          ...
          <button className="px-6 py-2 border-2 border-[#04407E] text-[#04407E] rounded-full font-semibold hover:bg-gray-50 transition-colors w-full">
            Apply Now
          </button>
        </GlassCard>
      </div>
...
"""
    ...
    # Insert components before PortalPreview export
    content = content.replace('export default function PortalPreview() {', new_components + '\nexport default function PortalPreview() {')

    # Add to menu array
    menu_search = "const menu = ["
    menu_replacement = """const menu = [
    ...
    # regex replace the entire menu array
    content = re.sub(r'const menu = \[.*?\];', menu_replacement, content, flags=re.DOTALL)
    
    # Add to Routes
    routes_replacement = """<Route path="/" element={<Dashboard />} />
    ...
    content = re.sub(r'<Route path="/" element=\{<Dashboard />\} />.*?<Route path="/emi" element=\{<EMISchedule />\} />', routes_replacement, content, flags=re.DOTALL)

    with open(filepath, 'w') as f:
        f.write(content)

insert_components('src/pages/PortalPreview.tsx')
