...
pages = {
    "InterestRates.tsx": """import { Percent, CheckCircle2 } from 'lucide-react';
...
imports = "import InterestRates from '@/pages/InterestRates';\n"

if "import InterestRates from" not in app_code:
    app_code = app_code.replace("import EMICalculator from '@/pages/EMICalculator';", imports + "import EMICalculator from '@/pages/EMICalculator';")

routes = "          <Route path=\"/interest-rates\" element={<InterestRates />} />\n"

if '<Route path="/interest-rates"' not in app_code:
    app_code = app_code.replace('<Route path="/emi-calculator" element={<EMICalculator />} />', routes + '<Route path="/emi-calculator" element={<EMICalculator />} />')
...
new_titles = "  '/interest-rates': 'Our Interest Rates',\n"
if "'/interest-rates'" not in title_code:
    title_code = title_code.replace("'/emi-calculator': 'EMI Calculator',", "'/emi-calculator': 'EMI Calculator',\n" + new_titles)
    with open(title_file, "w") as f:
        f.write(title_code)

print("Interest Rates page created and registered.")
